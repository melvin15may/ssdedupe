...
def get_components(edges, vertices=None):
    if vertices is None:
        vertices = pd.DataFrame(
            {'id': pd.concat((edges['id1'], edges['id2'])).unique()})
...
def components_dict_to_df(components):
    ...
    for id1 in components:
        deduped = np.append(deduped, [[id1, id2]
                                      for id2 in components[id1]], axis=0)
...
def merge(mapping_table, mapping_id,
    ...
    exact_columns_join = ' BINARY_CHECKSUM({}) = BINARY_CHECKSUM({}) '.format(','.join(
        ['t.{}'.format(x) for x in exact_columns]),
        ','.join(['t1.{}'.format(x) for x in exact_columns]))
