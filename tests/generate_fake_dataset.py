...
fake = Faker()
nicknames = pd.read_csv(path.join(path.dirname(
    path.abspath(__file__)), 'nicknames.csv'), skipinitialspace=True)
...
class Person(object):
    ...
    def munged_fname(self):
        ...
        if fname in nicknames.name.values and random.random() < .2:
            fname = random.choice(
                list(nicknames[nicknames.name == fname].nickname))
    ...
    def munged_sex(self):
        ...
        if random.random() < .001:
            return 'F' if self.sex == Sex.male else 'M'
    ...
    def munged_race(self):
        if random.random() < .2:
            return None
        if random.random() < .1:
            return random.choice(list(Race)).name
    ...
    def munged_ethnicity(self):
        if random.random() < .3:
            return None
        if random.random() < .1:
            return random.choice(list(Ethnicity)).name
    ...
    def _rand():
        ...
        ethn = np.random.choice(
            Ethnicity, 1, p=Ethnicity.__probabilities_by_race__(race))[0]
        # Let's just guess that 50% of hispanics have two last names, and a
        # small number otherwise:
...
def typo(s):
    ...
    while i < len(chars):
        ...
        if random.random() < .00033333 and 0 <= ord(chars[i].lower()) - ord('a') < 26:
            chars[i] = chr(((ord(chars[i].lower()) - ord('a') +
                             random.choice(range(1, 25))) % 26) + ord('a'))
        if random.random() < .00033333:
            chars.pop(i)
            i = i - 1
        i = i + 1
